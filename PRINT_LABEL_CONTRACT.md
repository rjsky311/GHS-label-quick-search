...
- A complete primary label includes product identifier, CAS, signal word, all GHS pictograms, H-statements, P-statements, responsible lab/supplier information or a clear missing-profile warning, trust footer, and optional QR support.
...
- Dense content must route users to a larger complete-label stock first, especially A4 Primary. Blocking print is acceptable only when no safe complete-label path is available from the current configuration.
...
- Dense shipped-container labels on regular large stock show a clear A4 Primary action instead of leaving the user at a disabled print dead end.
- A4 Primary allows dense complete labels within the documented statement threshold.
...
- Hydrochloric acid after switching to A4 Primary: print action becomes available; all pictograms remain visible in the live label preview.
