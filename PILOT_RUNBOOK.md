...
1. Confirm production health:
   - Use the current frontend and backend URLs in
...
2. Run or confirm the latest relevant gates:
   ...
   - During the temporary Zeabur rollback window only,
     `npm run qa:zeabur-deployment` remains an optional provider diagnostic
     when valid provider credentials are available.
