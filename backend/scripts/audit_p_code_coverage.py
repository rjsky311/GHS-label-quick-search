...
from p_code_coverage_audit import (  # noqa: E402
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Dry-run audit for P-code wording coverage. It checks observed "
            "PubChem/backend precautionary statement codes against the local "
            "English and Traditional Chinese wording dictionaries, and flags "
            "API payload fields that have already degraded to code-only text."
        )
    )
    parser.add_argument(
        "--input",
