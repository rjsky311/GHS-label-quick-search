...
| In-app correction intake | `Gate added` | Backend correction-request storage/API, admin queue, public in-app form entry, admin-only candidate evidence bundles, pending manual-entry conversion, and conversion trace-back to the originating correction request are in place | Backend tests, focused frontend tests, production search UI |
