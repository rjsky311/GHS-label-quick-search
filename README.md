...
- 最近前端驗證：`npm run test:i18n` → success；`npm test -- --runInBand` → 49 suites / 814 passed；`npm run test:print-contract` → 228 focused print/planner/renderer assertions；`npm run qa:print-pdf` → 30 print cases；`npm run build` → success，並透過 Vite `manualChunks` 拆分 vendor bundles。Production print QA 可用 GitHub Actions 的 `Production Print QA` workflow 執行 smoke/layer/full/all，並保留 JSON reports、screenshots、PDF artifacts 與 summary manifest。
