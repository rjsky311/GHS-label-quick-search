...
- Completed: expanded `npm run test:docs` to include the owner docs for data
...
| Inventory workbook audit | `Gate added` | Maintainer dry-run script now audits real lab workbooks for CAS cleanup, duplicates, seed-dictionary gaps, and review-only Chinese-name candidates; import handoff packets only into the review-only admin correction queue, where they now have a distinct triage focus, source badge, and expected/current-output context | `python backend/scripts/audit_inventory_workbook.py <xlsx>` + focused backend/frontend parser/admin tests |
