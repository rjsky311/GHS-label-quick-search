...
| Inventory workbook audit | `Gate added` | Maintainer dry-run script now audits real lab workbooks for CAS cleanup, duplicates, seed-dictionary gaps, and review-only Chinese-name candidates; use it only when a real workbook should become evidence for a parser/data-governance slice | `python backend/scripts/audit_inventory_workbook.py <xlsx>` + focused backend/frontend parser tests |
