...
- **Production search UI QA**: `npm run qa:production-search-ui` uses deployed Chrome to search Hydrochloric Acid, inspect the result-row pictogram strip, expand alternate classifications, and open the detail modal classification comparison table. It saves result, expanded-classification, and detail-modal screenshots and fails on unreadable/missing pictogram strips or vertical action-button regressions.
