...
| Batch-wide state, visible filtered state, review reasons, and next actions are clear. | Passed | `ResultsTable` keeps batch-wide counts separate from visible scope, uses one review action queue as the batch-level filter surface, and focused `ResultsTable` tests cover filtering and summary behavior. |
| Multiple-GHS rows show system-suggested vs user-confirmed state and route to the chooser. | Passed | Result rows and export rows preserve multiple-GHS status; multiple-GHS chips open the existing classification chooser; the batch summary now also promotes multiple-GHS rows into a dedicated confirmation callout before label/export handoff; production search UI QA checks the deployed action. |
...
| Exports preserve trust/review context and remain readable for lab-manager triage. | Passed | CSV/XLSX share trust headers, `Review Signal Count`, and `Primary Review Action`; XLSX includes `GHS Results`, `Ready Rows`, `Needs Review`, `Unresolved`, and `Pilot Summary`; the summary separates original/visible row counts, unique review rows, total review signals, and overlapping review signals; export preview shows ready/review/unresolved counts plus the review-action handoff explanation before download. |
