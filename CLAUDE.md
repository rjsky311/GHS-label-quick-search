...
order, and done criteria. The print-workflow refactor baseline is pinned in
...
- **Production search UI QA**: `npm run qa:production-search-ui` uses deployed Chrome to search Hydrochloric Acid, inspect result-row/expanded/detail/mobile pictogram and trust surfaces, and verify Detail/Prepare Solution modal keyboard containment. It writes full JSON evidence and screenshots while printing a compact console summary for CI review.
